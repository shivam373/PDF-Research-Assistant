...
Fixes applied:
  1. **bold** markdown now renders as actual bold in chat
  2. Focus mode toggle: 2-panel view (PDF viewer | Chat) vs full 3-panel
  3. Selected book shown as a styled pill badge
  4. Margins reduced throughout
  5. Closing directory dialog on first launch exits cleanly (no loop)
"""
...
from PyQt6.QtWidgets import (
    ...
    QTextEdit, QTextBrowser, QSpinBox, QFileDialog, QMessageBox,
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        # Main 3-panel splitter (default)
        self.main_splitter = QSplitter(Qt.Orientation.Horizontal)
        self.main_splitter.setHandleWidth(2)
        self.setCentralWidget(self.main_splitter)

        # ── LEFT: search + results + chat ─────────────────────────────────────
        ...
        # Top bar: dir label + change btn + focus toggle
        top_row = QHBoxLayout()
        ...
        top_row.addWidget(btn_change)

        self.btn_toggle = QPushButton("⛶ Focus")
        self.btn_toggle.setObjectName("toggleBtn")
        self.btn_toggle.setToolTip("2-panel mode: PDF viewer + Chat only")
        ...
        # Book badge (shown below list when a PDF is selected)
        ...
        self.main_splitter.setSizes([390, 1050])
    ...
    def _toggle_focus_mode(self):
        ...
        if self._focus_mode:
            ...
            self.btn_toggle.setToolTip("Back to full search mode")
            if not hasattr(self, "_focus_splitter"):
                self._build_focus_panel()
            self.main_splitter.hide()
            self.setCentralWidget(self._focus_splitter)
            self._focus_splitter.show()
        else:
            self.btn_toggle.setText("⛶ Focus")
            self.btn_toggle.setToolTip("2-panel mode: PDF viewer + Chat only")
            self._focus_splitter.hide()
            self.setCentralWidget(self.main_splitter)
            self.main_splitter.show()

    def _build_focus_panel(self):
        """Build the 2-panel focus view once (PDF viewer | chat)."""
        self._focus_splitter = QSplitter(Qt.Orientation.Horizontal)
        self._focus_splitter.setHandleWidth(2)
        self._focus_splitter.setStyleSheet(APP_STYLE)

        # Left side: PDF viewer (same widget)
        self._focus_splitter.addWidget(self.pdf_viewer)

        # Right side: compact chat
        chat_panel = QWidget()
        cv = QVBoxLayout(chat_panel)
        cv.setContentsMargins(6, 6, 6, 6)
        cv.setSpacing(4)

        # Badge showing which book is open
        fb_row = QHBoxLayout()
        self.lbl_focus_badge = QLabel("No PDF open")
        self.lbl_focus_badge.setObjectName("bookBadge")
        fb_row.addWidget(self.lbl_focus_badge)
        fb_row.addStretch()

        btn_back = QPushButton("⊞ Back to Search")
        btn_back.setObjectName("secondary")
        btn_back.clicked.connect(self._toggle_focus_mode)
        fb_row.addWidget(btn_back)
        cv.addLayout(fb_row)
        ...
        q2 = QHBoxLayout()
        q2.addWidget(self.question_input)
        q2.addWidget(self.btn_ask)
        cv.addLayout(q2)

        self._focus_splitter.addWidget(chat_panel)
        self._focus_splitter.setSizes([960, 480])

    # ════════════════════════════════════════════════════ directory / index ══

    def _pick_directory(self):
        """Prompt for a PDF directory. On first-launch cancel → offer to quit."""
    ...
    def _start_indexing(self, directory: str):
        ...
        self.btn_ask.setEnabled(False)
        self.pdf_list.clear()
        self._append_chat("system", f"🔄 Indexing PDFs …")
    ...
    def _do_search(self):
        ...
        self._append_chat("system", f'🔍 {len(self.pdf_results)} PDF(s) found for "{query}".\n')
    ...
    def _set_book_badge(self, name: str):
        if name:
            ...
            self.lbl_book_badge.setText(f"📖  {display}")
            self.lbl_book_badge.setVisible(True)
            if hasattr(self, "lbl_focus_badge"):
                self.lbl_focus_badge.setText(f"📖  {display}")
        else:
            self.lbl_book_badge.setVisible(False)
            if hasattr(self, "lbl_focus_badge"):
                self.lbl_focus_badge.setText("No PDF open")
    ...
    def _markdown_to_html(self, text: str) -> str:
        """Convert **bold**, `code`, and [cite, p.N] in escaped text to HTML."""
        # HTML-escape first
        text = self._esc(text)
        # **bold**
        ...
        # [filename, p.N] → styled citation pill
        text = re.sub(
            r"\[([^\]]+?,\s*p\.\d+)\]",
    ...
    def _remove_thinking_line(self):
        """Delete the '⏳ Thinking …' paragraph from the chat."""
